...
// 아이디 찾기
export async function search_email_id(body: { phone_number: string }) {
  return send(SEARCH_EMAIL_ID, 'POST', body, false, false);
}

// 비밀번호 찾기
export async function search_password(body: { email_id: string }) {
...
//인벤토리 보유 아이템 목록 조회
export async function get_my_items(body: { cate_group_code: string }) {
...
// 캐시백 이벤트 아이템 수령
export async function cashback_item_receive(body: { event_tmplt_seq: string }) {
