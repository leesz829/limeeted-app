...
export const Login01 = (props : Props) => {
	...
	const loginProc = async () => {
		...
		axios.post(properties.api_domain + '/join/getLoginchk/', {
			kakao_id : id
			, password : password
		})
		.then(function (response) {
			...
			if(resultCode == "0000" || (resultCode == "0001" && (status == "PROCEED" || status == "APROVAL"))) {
				...
				AsyncStorage.setItem('memberInterviewList', JSON.stringify(response.data.memberInterviewList));
