...
export const Preference : FC<Props> = (props) => {
	...
	const [wantAgeMin, setWantAgeMin] = React.useState<any>(props.idealTypeData.want_age_min);
	const [wantAgeMax, setWantAgeMax] = React.useState<any>(props.idealTypeData.want_age_max);
	...
	// 셀렉트 박스 콜백 함수
	const busi1CallbackFn = (value : string) => { setWantBus1(value); };
	const busi2CallbackFn = (value : string) => { setWantBus2(value); };
	const busi3CallbackFn = (value : string) => { setWantBus3(value); };
	const jobCd1CallbackFn = (value : string) => { setWantJob1(value); };
	const jobCd2CallbackFn = (value : string) => { setWantJob2(value); };
	const jobCd3CallbackFn = (value : string) => { setWantJob3(value); };
	const wantPerson1CallbackFn = (value : string) => { setWantPerson1(value); };
	const wantPerson2CallbackFn = (value : string) => { setWantPerson2(value); };
	...
	// 업종 상태 관리
	React.useEffect(() => {	if(wantBusiness1 != '') { getJobCodeList('01'); } }, [wantBusiness1]);
	React.useEffect(() => {	if(wantBusiness2 != '') { getJobCodeList('02'); } }, [wantBusiness2]);
	React.useEffect(() => {	if(wantBusiness3 != '') { getJobCodeList('03'); } }, [wantBusiness3]);
		...
		<View style={layoutStyle.flex1}>
			...
			<Modalize
				...
				<View style={modalStyle.modalHeaderContainer}>
							...
							<ScrollView contentContainerStyle={modalStyle.modalBody}>
								...
								<SpaceView mb={32}>
									...
									<SpaceView mb={24} viewStyle={styles.halfContainer}>
										<View style={styles.halfItemLeft}>
											<CommonSelect label={'업종'} items={busiGrpCdList} selectValue={wantBusi1} callbackFn={busi1CallbackFn} />
										</View>
									...
									<SpaceView mb={24} viewStyle={styles.halfContainer}>
										<View style={styles.halfItemLeft}>
											<CommonSelect label={'업종'} items={busiGrpCdList} selectValue={wantBusi2} callbackFn={busi2CallbackFn} />
										</View>
									...
									<SpaceView viewStyle={styles.halfContainer}>
										<View style={styles.halfItemLeft}>
											<CommonSelect label={'업종'} items={busiGrpCdList} selectValue={wantBusi3} callbackFn={busi3CallbackFn} />
										</View>
					...
					<SpaceView mb={16}>
						<CommonBtn value={'저장'} type={'primary'} onPress={() => { console.log('asdsaldkmlmk'); }} />
					</SpaceView>
